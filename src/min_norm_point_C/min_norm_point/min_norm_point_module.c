...
// Function to know whether a polyhedron is fully dimensional (exists x such that V * x <= s - res)
...
// Main function to know whether a polyedron defined by (V,s) is full-dimensional (3 methods: GLPK, SKM, Mine)
...
// Function to know whether q is minor regarding p in polyhedron (V,s) (q is supposed to be in it, and p not) - with GLPK feasibilty
...
// Function to know whether q is minor regarding p in polyhedron (V,s) (q is supposed to be in it, and p not) - with SKM algorithm
...
// Function to know whether q is minor regarding p in polyhedron (V,s) (q is supposed to be in it, and p not) - with Mine algorithm (SKM-like)
...
// Function to know whether q is minor regarding p in polyhedron (V,s) (q is supposed to be in it, and p not) - pure geometry building ortho. polyhedral cone
...
// Main function to know whether q is minor regarding p in polyhedron (V,s) (q is supposed to be in it, and p not) (4 methods: GLPK, SKM, Mine, Cone)
...
// Function to know whether a vector couple is necessary
...
bool project_q_0dr1(
    ...
    const double res, const int max_iter, int *nb_recursions
) {
    *nb_recursions += 1;
    if (max_iter >= 0 && *nb_recursions >= max_iter) { // maximum number of iterations reached!
        //fprintf(stderr, "Warning : Maximum number of iterations reached! Initial point returned.\n");
        for (int j = 0; j < cols; j++) {
            q[j] = 0;
        }
        return true;
...
double* algo_0dr1(const double* V, const double* s, int rows, int cols, const double* p, const double res, const int max_iter) {
...
double* minimum_norm_points_to_polyhedra(
    ...
    for (int i = 0; i < n_points; i++) {
        ...
        for (int j = 0; j < n_polyhedra; j++) {
            ...
            double* qij = (double*)algo_0dr1((const double*)l_V_j, (const double*)l_s_j, (int)rows_j, cols, (const double*)points_i, res, max_iter);
...
static PyObject* minimum_norm_points_to_polyhedra_py(PyObject *self, PyObject *args) {
    ...
    int n_polyhedra, n_points, cols, max_iter;
    double res;

    if (!PyArg_ParseTuple(args, "O!O!O!O!iiidi", 
                          &PyArray_Type, &V, 
                          &PyArray_Type, &s, 
                          &PyArray_Type, &points, 
                          &PyArray_Type, &Lrows, 
