...
from . import min_norm_point_module
...
def minimum_norm_points_to_polyhedra(
    ...
    * max_iter: maximum number of iterations or recursions allowed for the min-norm point 
     algotithm. If negative, then no limit.\n
    ...
    if res is None:
        res = min(1e-6, mres(V[0], 1 + max(0, int(np.ceil(np.log10(V[0].shape[-1] * np.max([np.max(np.abs(V[i])) for i in range(len(V))])))))))
    ...
    min_n_pts = min_norm_point_module.minimum_norm_points_to_polyhedra(
        ...
        int(n_polyhedra), int(n_points), int(cols), 
        float(res), int(max_iter)
    )
